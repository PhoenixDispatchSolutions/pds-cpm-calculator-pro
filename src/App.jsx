...
function MonthlyCalculator({ cacheKey = "monthly_v1" }) {
  ...
  const totals = useMemo(() => {
    ...
    const rpm = parsed.miles > 0 ? parsed.gross / parsed.miles : NaN;
    const cpm = isFinite(rpm) ? rpm * 100 : NaN;
    const beRpm = parsed.miles > 0 ? expenses / parsed.miles : NaN;
    const beCpm = isFinite(beRpm) ? beRpm * 100 : NaN;
      ...
      <div className="space-y-6">
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Stat label="Actual CPM" value={isFinite(totals.cpm) ? totals.cpm.toFixed(1) : "—"} />
            <Stat label="Break-even CPM" value={isFinite(totals.beCpm) ? totals.beCpm.toFixed(1) : "—"} />
