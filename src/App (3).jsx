...
/**
 * PDS CPM Calculator Pro (v3+) — Targeted Fix Build
 * - Monthly calculator math fixed (CPM/Break-even in $/mile)
 * - Per-Load: removed Loads-per-Month projection UI
 * - Landing cards: gradient border + hover scale/glow
 * - NEW: Clear All button on both calculators (clears localStorage for that calc)
 */
...
function PerLoadCalculator({ cacheKey = "perload_v1" }) {
  ...
  const clearAll = () => {
    ["gross","acc","truck","trail","ins","mpg","miles","state","price","driver","maint","misc"].forEach(k => localStorage.removeItem(`${cacheKey}_${k}`));
    reset();
  };
      ...
      <div className="md:col-span-2 space-y-6">
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 space-y-4">
          ...
          <div className="flex gap-3 items-end">
            ...
            <button onClick={clearAll} className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white">Clear All</button>
          </div>
        </div>
      </div>
...
function MonthlyCalculator({ cacheKey = "monthly_v1" }) {
  ...
  const totals = useMemo(() => {
    ...
    const cpm = p.miles > 0 ? p.gross / p.miles : 0;   // revenue per mile
    const beCpm = p.miles > 0 ? expenses / p.miles : 0;// cost per mile
    const margin = p.gross > 0 ? (net / p.gross) * 100 : 0;

    return { fuelCost, expenses, net, cpm, beCpm, margin };
  ...
  const clearAll = () => {
    ["gross","miles","truck","trail","ins","mpg","state","price","driver","maint","misc"].forEach(k => localStorage.removeItem(`${cacheKey}_${k}`));
    reset();
  };
